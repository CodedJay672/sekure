...
const TableDetailComponent: React.FC<TableComponentProps> = ({ variant, columns, data }) => {
    ...
    <div className={`${variant === 'big' ? 'mt-4' : ''} w-full flex-1 overflow-auto`}>
      <table className="w-full min-w-max text-[11px] text-left text-dark3">
        ...
        <tbody>
          {data.length > 0 ? (
            data.map((row, idx) => (
              <tr key={idx} className={`bg-white dark:bg-gray-800 ${idx % 2 === 0 ? 'bg-gray-50 dark:bg-gray-900' : ''} text-[11px] hover:bg-[#F3F3F3] cursor-pointer`} onClick={() => handleClick(row.id)}>
                {columns.map((column) => (
