...
const UserCard: React.FC<UserCardProps> = ({ name, poste, email, parte }) => {
  ...
  return (
    <div className={`w-[473px] ${isOpen  ? 'max-h-[300px]' : 'h-auto' } py-[18px] px-[15px] border rounded-[14px]`}>
      <div className='w-full flex gap-5 cursor-pointer' onClick={() => setIsOpen(!isOpen)}>
        <div className='w-[48px] h-[48px] rounded-full bg-primary' />
        <div className='flex-1 flex items-center'>
          <div className='flex-1 flex flex-col gap-[2px]'>
            <div>
              <span className='text-[13px] leading-[17px] mr-[10px] font-semibold'>{name}</span>
              <span className='inline-block text-[10px] leading-[17px] px-[6px] bg-[#F5F5F5]'>En revue</span>
            </div>
            <div>
              <span className='text-[12px] leading-[24px] text-[#242424]'>{poste},</span>
              <span className='text-[12px] leading-[24px] text-[#242424]'> {email}</span>
            </div>
          </div>
      ...
      <div className={`${isOpen ? 'max-h-[270px] mt-[25px]' : 'max-h-0'} transition-all overflow-hidden duration-300 ease-in-out`}>
